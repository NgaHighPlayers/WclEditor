...
import { CombatEvent, extractPlayers, UnitReplace } from './events';
...
const fromEvents = {
  immediate: (
    events: CombatEvent[],
    { offset, replaces }: ReplaceOption,
    writeTo: string
  ) => {
    const lines = events.map((e) => e.toLine(offset || 0, replaces).line);
    fs.appendFileSync(writeTo, `${lines.join('\r\n')}\r\n`);
  },
  sync: async (
    events: CombatEvent[],
    { replaces }: ReplaceOption,
    writeTo: string,
    flushInterval = 30
  ) => {
...
const fromLog = {
  ...
  sync: async (
    ...
    fromEvents.sync(events, replaceOption, writeTo, flushInterval);
  },
};
